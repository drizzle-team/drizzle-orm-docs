...
# Meet Drizzle Studio
🖥 Drizzle Studio is a new way for you to explore SQL databases on Drizzle projects.
...
You can launch studio with the `port` cli flag to customize the process port and with the `verbose` flag for extended SQL statements logging.
```bash

...
If you're expecting values above 2^31 but below 2^53, you can utilise `mode: 'number'` and deal with javascript numbers as opposed to bigints.
<Section>
```typescript
...
### serial
`serial` `serial4`  
Auto incrementing 4-bytes integer, a notational convenience for creating unique identifier columns (similar to the `AUTO_INCREMENT` property supported by some other databases). 
...
### smallserial
`smallserial` `serial2`  
Auto incrementing 2-bytes integer, a notational convenience for creating unique identifier columns (similar to the `AUTO_INCREMENT` property supported by some other databases). 
...
### bigserial
`bigserial` `serial8`  
Auto incrementing 8-bytes integer, a notational convenience for creating unique identifier columns (similar to the `AUTO_INCREMENT` property supported by some other databases).
...
You can define the `{ enum: ["value1", "value2"] }` config to infer `insert` and `select` types. This **won't** check runtime values.
<Section>
```typescript
...
### char
`character(n)` `char(n)`  
Fixed-length, blank padded character string which can store strings up to **`n`** characters (not bytes).

For more info please refer to the official PostgreSQL **[docs.](https://www.postgresql.org/docs/current/datatype-character.html)**

You can define the `{ enum: ["value1", "value2"] }` config to infer `insert` and `select` types. This **won't** check runtime values.

The `length` parameter is optional according to the PostgreSQL docs.
<Section>
```typescript
...
### numeric
`numeric` `decimal`  
Exact numeric with selectable precision. It can store numbers with a very large number of digits, up to 131072 digits before the decimal point and up to 16383 digits after the decimal point.
...
You can specify `.$type<..>()` for json object inference. This **won't** check runtime values. 
It provides compile time protection for default values, as well as insert and select schemas.
```typescript
...
### Not null
The `NOT NULL` constraint dictates that the associated column may not contain a `NULL` value.

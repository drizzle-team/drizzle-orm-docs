...
<Callout type="info" emoji="ℹ️">
	...
	All JSON functions currently throw an error if any of their arguments are BLOBs. This happens because BLOBs 
...
You can specify `.$type<..>()` for blob inference. This **won't** check runtime values. 
...
SQLite does not have native a `boolean` data type. You can however specify `integer` column to be in a `boolean` mode. 

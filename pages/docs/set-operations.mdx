...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    const result = db.select({ name: users.name }).from(users)
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    const result = db.select({ transaction: onlineSales.transactionId }).from(onlineSales)
            .unionAll(
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <Tab>
    ...
    const depB = pgTable('department_b_courses', {
        studentId: integer('student_id'),
        courseName: int('course_name', { length: 256 }).notNull(),
    });
    ```
    </CodeTab>
  </CodeTabs>
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    const result = db.select({ 
        productId: regularCustomerOrders.productId,
        quantityOrdered: regularCustomerOrders.quantityOrdered }
    ).from(regularCustomerOrders).intersectAll(
        db.select({ 
                productId: vipCustomerOrders.productId,
                quantityOrdered: vipCustomerOrders.quantityOrdered 
            }).from(vipCustomerOrders)
  ...
  <Tab>
	...
	<CodeTab>
	```typescript copy
    import { intersectAll } from 'drizzle-orm/sqlite-core'
    import { regularCustomerOrders, vipCustomerOrders } from './schema'
    ...
    <CodeTab>
	```typescript copy
    const regularCustomerOrders = sqliteTable('regular_customer_orders', {
        customerId: int('customer_id').primaryKey(),
        productId: int('product_id').notNull(),
        quantityOrdered: int('quantity_ordered').notNull(),
    });
    
    const vipCustomerOrders = sqliteTable('vip_customer_orders', {
        customerId: int('customer_id').primaryKey(),
        productId: int('product_id').notNull(),
        quantityOrdered: int('quantity_ordered').notNull(),
    });
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    const result = await db.select({ courseName: depA.projectsName })
                        .from(depA)
                        .except(
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    const result = await db.select({ 
        productId: regularCustomerOrders.productId,
        quantityOrdered: regularCustomerOrders.quantityOrdered }
    ).from(regularCustomerOrders).exceptAll(
        db.select({ 
                productId: vipCustomerOrders.productId,
                quantityOrdered: vipCustomerOrders.quantityOrdered 
            }).from(vipCustomerOrders)
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
	<CodeTab>
	```typescript copy
    import { exceptAll } from 'drizzle-orm/sqlite-core'
    import { regularCustomerOrders, vipCustomerOrders } from './schema'

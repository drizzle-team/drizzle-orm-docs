...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    ```
    ```sql
    (select "name" from "sellers") union (select "name" from "customers") limit $1
    ```
    </CodeTab>
    <CodeTab>
    ```ts copy
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    ```
    ```sql
    (select "transaction_id" from "online_sales") union all (select "transaction_id" from "in_store_sales")
    ```
    </CodeTab>
    <CodeTab>
    ```ts copy
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    ```
    ```sql
    (select "course_name" from "department_a_courses") intersect (select "course_name" from "department_b_courses")
    ```
	</CodeTab>
    <CodeTab>
    ```typescript copy
  ...
  <Tab>
    ...
    ```
    ```sql
    (select `projects_name` from `department_a_projects`) intersect (select `projects_name` from `department_b_projects`)
    ```
	</CodeTab>
    <CodeTab>
    ```typescript copy
...
<Tabs items={["PostgreSQL", "MySQL", "SQLite"]}>
  ...
  <CodeTabs items={["import-pattern", "builder-pattern", "schema.ts"]}>
    ...
    ```
    ```sql
    select "projects_name" from "department_a_projects" except select "projects_name" from "department_b_projects"
    ```
	</CodeTab>
    <CodeTab>
    ```ts copy
...
n this scenario, you want to find products that are exclusively ordered by regular customers and not 

...
<Callout>
	Note: The Batch API is only supported in the **[`libsql`](./installation-and-db-connection/sqlite/turso)**
	driver and will soon be supported in **[`D1`](./docs/installation-and-db-connection/sqlite/d1)**
</Callout>

**LibSQL Batch API explanation**:
_[source](https://docs.turso.tech/reference/client-access#batches)_
...
Drizzle ORM provides APIs to run SQL statements in batch for `LibSQL` and `D1` (__WIP__):
```ts
...
```
The type for `batchResponse` in this example would be:
```ts
type BatchResponse = [
	{
		id: number;

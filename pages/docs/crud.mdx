...
Insert user and if user with `id=1` exists - update his name to Jogn
```typescript copy
...
Insert John, and if user with primaryKey equals 1 exists, do nothing
```typescript
await db.insert(users)
...
Insert John, and if user with `id=1` exists, do nothing
```typescript
await db.insert(users)
  .values({ id: 1, name: 'John' })

...
grouping and filterin with `groupBy` and `having` respectfully, just like you do in SQL
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  <Tab>
    <Section>
    ```typescript copy {10-11}
    ...
    await db.select({ count: sql<number>`count(*)` }).from(product);

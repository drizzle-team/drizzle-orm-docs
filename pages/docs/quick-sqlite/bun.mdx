...
Drizzle ORM natively supports the **[`bun:sqlite`](https://bun.sh/docs/api/sqlite)** module and it's crazy fast 🚀  


We embrace SQL dialects as well as dialect specific drivers and syntax and unlike any other ORM, for synchronous drivers like `bun:sqlite` we offer both **async** and **sync** APIs. We also mirror the most popular SQLite-like query methods, such as `all`, `get`, `values` and `run`.

...
Drizzle ORM embraces SQL dialects, their specific drivers and syntax and unlike any other ORM, for synchronous drivers like `better-sqlite3` we offer both **async** and **sync** APIs. We also mirror the most popular SQLite-like query methods, such as `all`, `get`, `values` and `run`.

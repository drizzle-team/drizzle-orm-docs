...
# SQL Select
Drizzle provides you the most SQL-like way to query your relational database, while remaining type-safe and composable.
...
```ts
await db.select().from(users).where(equals(users.id, 42));
```
...
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  <Tab>
    ...
    const result = await db.select({ count: sql<number>`cast(count(*) as int)` }).from(product);
    /* result type is { count: number }[] */

    await db.select({ count: sql<number>`cast(count(*) as int)` }).from(product).where(gte(product.unitPrice, 4));

    ```
    ```sql
    select cast(count(*) as int) from "product";
    select cast(count(*) as int) from "product" where "unit_price" >= 4;
    ```
    </Section>
  </Tab>
  <Tab>
...
<Callout type="info">
Casting to `int` is necessary because `count()` returns `bigint` in PostgreSQL and `decimal` in MySQL, which are treated as string values instead of numbers.
Alternatively, you can use [`.mapWith(Number)`](/docs/sql#sqlmapwith) to cast the value to a number at runtime.
</Callout>

Lets take a quick look at grouping and filtering grouped values using a `having` clause:
<Section>
```typescript
import { gt } from 'drizzle-orm';

await db.select({
  count: sql<number>`count(${user.id})::int`,
  city: user.city,
})
  .from(user)
  .groupBy(({ city }) => city)
...
```
```sql
select cast(count("id") as int), "city" from "users"
  group by "city"

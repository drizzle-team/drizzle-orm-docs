...
We natively support most query feature capabilities for every dialect and whatever we don't yet support can be achieved using our powerful **[`sql`](/docs/sql)** operator.

## Basic and partial select
Retrieve a list of all users, and you will receive a typed result set:
...
For SQL tables with many columns, you might not want to select all of them due to performance or security reasons.


You can omit them by using our partial query syntax, which will generate a partial SQL select and automatically map the results.
...
With partial select you can apply SQL transformations with the `sql` operator:
...
Any filter operator is an `sql` operator under the hood. For full SQL potential 
you can utilise it directly and build type safe and future safe queries.

You can safely alter the schema, rename tables and columns, and these changes will automatically reflect in queries, unlike with regular raw SQL string queries.
...
## Combining filters
You can logically combine filter operators with conditional `and` and `or` operators, like this:
<Section>
```typescript copy
...
Drizzle ORM supports the `DISTINCT ON` PostgreSQL operator too.
<Callout>
...
## Order By
You can sort results with the `orderBy` operator:
<Section>
```typescript
...
## WITH clause
The SQL `with` clause is a statement scoped view, useful for organizing complex queries like the following:
<Section>
```typescript copy
...
With our powerful `sql` operator you can infer aggregation functions return types using `sql<number>` syntax:
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  ...
  <Tab>
    <Section>
      ```typescript copy {10,13}
      import { mysqlTable, bigint, text, double } from 'drizzle-orm/mysql-core';
      import { gte } from 'drizzle-orm';
...
Lets have a quick look on how to group and filter grouped using `having`:
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  <Tab>
    <Section>
...
Here's a more advanced example. This query will return a list of orders with their details,
and will calculate total price, total quantity and total products count for each order.
```typescript copy

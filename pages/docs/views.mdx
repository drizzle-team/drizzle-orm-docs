...
<Callout type="warning" emoji="⚠️">
  Views are currently only implemented in `drizzle-orm`. `drizzle-kit` does not support views yet. 
...
## Views declaration
There are several ways you can declare views with Drizzle ORM.  

You can declare views that have to be created or you can declare views that already exist in the database. 

You can declare views statements with an inline `query builder` syntax, with the `standalone query builder` and with raw `sql` operators. 

When views are created with either the inline or standalone query builder, the view columns schema will be automatically inferred. 
However when you use `sql` you have to explicitly declare the view columns schema.
...
If you need a subset of columns you can use the `.select({ ... })` method in the query builder, like this:
<Section>
  ```ts {4-6}
...
You can also declare views using the `standalone query builder`. It works exactly the same way:
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  <Tab>
    <Section>
...
Whenever you need to declare a view using a syntax that is not supported by the query builder, 
you can directly use the `sql` operator and explicitly specify the view columns schema.
...
When you're provided with read only access to an existing view in the database, you should use the `.existing()` view configuration. 
`drizzle-kit` will ignore it and will not generate a `create view` statement in the generated migration.
```ts
export const user = pgTable("user", {
  id: serial("id"),
  name: text("name"),

...
and [`materialized`](https://www.postgresql.org/docs/current/sql-creatematerializedview.html) views and Drizzle ORM provides APIs for both of them
...
When views are created with either inlined or standalone query builders - view columns schema will be automatically infered, 
...
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  <Tab>
    <Section>
      ...
      export const customersView = pgView("customers_view").as((qb) => qb.select().from(user).where(eq(user.role, "cusotmer")));
      ```
      ```sql
  ...
  <Tab>
    <Section>
      ...
      export const customersView = mysqlView("customers_view").as((qb) => qb.select().from(user).where(eq(user.role, "cusotmer")));
      ```
      ```sql
      CREATE VIEW "user_view" AS SELECT * FROM "user";
      CREATE VIEW "customers_view" AS SELECT * FROM "user" WHERE "role" = 'customer';
...
If you need a subset of columns - you can use `.select({ ... })` method in query builder
<Section>
  ```ts {4-6}
...
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  <Tab>
    <Section>
      ...
      export const customersView = pgView("customers_view").as(qb.select().from(user).where(eq(user.role, "cusotmer")));
      ```
      ```sql
      CREATE VIEW "user_view" AS SELECT * FROM "user";
      CREATE VIEW "customers_view" AS SELECT * FROM "user" WHERE "role" = 'customer';
  ...
  <Tab>
    <Section>
      ...
      export const customersView = mysqlView("customers_view").as(qb.select().from(user).where(eq(user.role, "cusotmer")));
      ```
      ```sql
      CREATE VIEW "user_view" AS SELECT * FROM "user";
      CREATE VIEW "customers_view" AS SELECT * FROM "user" WHERE "role" = 'customer';
...
You can then refresh materialized vies in the application runtime
```ts copy

...
# Joins [SQL]
The join clause in SQL is used to combine 2 or more tables, based on related columns between them.
Drizzle ORM's join syntax strikes a balance between SQL-likeness and type safety.

## Join types
Drizzle ORM has APIs for `INNER JOIN`, `FULL JOIN`, `LEFT JOIN` and `RIGHT JOIN`.
Let's examine some examples using the following table schemas:
```typescript copy
...
It's important to remember that when using the sql operator for partial selection fields and aggregations, you should use `sql<type | null>` for proper result type inference. This responsibility is on you.
<Section>
```typescript copy
...
To avoid a plethora of nullable fields when joining tables with many columns, you can use our nested select object syntax. Our smart type inference will make the entire object nullable instead of making each table field nullable.
<Section>
```typescript copy
await db.select({
  userId: users.id,
...
## Aliases & Selfjoins
Drizzle ORM supports table aliases, which are especially useful for self-joins
...
export const user = pgTable("user", {
  ...
  name: text("name").notNull(),
  parentId: integer("parent_id").notNull().references((): AnySQLiteColumn => user.id)
});
```
...
You have the flexibility to process results as desired. Here's an example of mapping many-to-one relational data:
```typescript

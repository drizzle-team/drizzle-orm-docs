...
The `table creator` operator lets you define customise table names.  
...
## Printing SQL query
You can print SQL queries with the `db` instance or by using the **[`standalone query builder`](#standalone-query-builder)**.
```typescript copy
const query = db

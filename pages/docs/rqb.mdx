...
When using the **mysql2** driver with a regular **MySQL** database — you should specify `mode: "default"`.
...
Example of `one-to-one` relations between users and users who invited them to the service, self reference:
...
Another example would be a user having profile information stored in a separate table:
...
### One-to-many
Drizzle ORM provides you an API to define `one-to-many` relations between tables with the `relations` operator. 

Example of a `one-to-many` relation between users and the posts they've written:
...
Example of a `many-to-many` relation between users and groups:
...
What this means is that `relations` and foreign keys can be used together, but they are not dependent on each other.
...
<Callout type="info" emoji="ℹ️">
	The `drizzle` import path depends on the **[database driver](/docs/installation-and-db-connection)** you're using.
</Callout>
<CodeTabs items={["index.ts", "schema.ts"]}>
<CodeTab>
```ts
...
The `with` operator lets you combine data from multiple related tables and properly aggregate results.
...
For any nested `with` queries Drizzle will infer types using [Core Type API](./goodies#type-api).
...
The `columns` parameter lets you include or omit columns you want to get from the database.
...
If you include the `name` field and exclude the `id` field, the `id` exclusion becomes redundant because all fields other than name would be excluded anyway.  
...
### Select filters
Just like in our SQL-like query builder, the relational queries API allows you to define filters and conditions using our list of **[`operators`](/docs/operators)**.  
...
Find the post with `id=1` and comments that were created before the specified date:
```typescript copy
...
### Limit & Offset
Drizzle ORM provides a `limit` & `offset` API for queries and for the nested entities.
...
### Order By
Drizzle provides an API for ordering in the relational query builder.  

You can use the same ordering **[core API](/docs/crud#order-by)** or use
...
The relational query API lets you add custom additional fields. 
...
`lowerName` as a key will be included to all fields in the returned object.

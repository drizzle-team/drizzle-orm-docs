...
## Foreign keys
Drizzle ORM lets you define [foreign keys](indexes-constraints#foreign-key) in SQL schema with `.references()` and `foreignKey()` operators.  
While that's very useful for the database level constraint checks, it is not necessary for `relations` module to work.  

These two examples will work exactly the same from the perspective of Drizzle relational queries.
...
That lets you use `relational queries` on the databases, that do not support foreign keys, like PlanetScale. 
...
<Callout type="warning" emoji="⚠️">
	As of now aggregations are not supported in `extras`, please use [`core queries`](./crud) for that
</Callout>

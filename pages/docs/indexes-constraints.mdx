...
The `DEFAULT` clause specifies a default value to use for the column if no value is provided by the user when doing an `INSERT`.
...
or update a record without providing a value to this field.
...
Both the `UNIQUE` and `PRIMARY KEY` constraints ensure uniqueness for a column or set of columns.
...
If you define a `CHECK` constraint on a column it allows only certain values for this column.  
...
If you want to create a self-reference, due to TypeScript limitations, you either need to explicitly set the return type for the reference callback or use a standalone `foreignKey` operator.
...
Drizzle ORM provides APIs for both `index` and `unique index` declaration:
...
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
  <Tab>
    ...
    <Callout type="warning" emoji="⚠️">
      As of now `drizzle-kit` only supports the index `name` and `on()` params.
    </Callout>

    Drizzle ORM provides a set of all the params for index creation:

...
Drizzle Kit supports multiple formats for the configuration file, including `.ts`, `.js`, and `.json`.
...
**Usage**
<Tabs items={["single", "multiple", "glob"]}>
<Tab>
```ts
export default {
  schema: "./schema.ts",
...
a database connection url is not possible. For example, [Turso](https://turso.tech)

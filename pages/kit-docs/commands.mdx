...
If you want to use a custom named config file.
...
The `drizzle-kit introspect:{dialect}` command lets you pull DDL from an existing database and generate `schema.ts` file in a matter of seconds.  
...
`drizzle-kit drop` lets you delete previously generated migrations from the migrations folder.
...
We're rapidly evolving Drizzle Kit APIs and from time to time there's a need to upgrade the underlying metadata structure.
...
## Check
`drizzle-kit check:{dialect}` is a very powerful tool for you to check consistency of your migrations. It's extremely useful when you have multiple people on the project, altering the database schema on different branches. Drizzle Kit will check for all collisions and inconsistencies.
...
## Drizzle Studio [NEW]
`drizzle-kit studio` lets you launch the **[Drizzle Studio](/drizzle-studio/overview)** database browser locally from you config file.  

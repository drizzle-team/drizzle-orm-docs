...
Either **[`uri`](./conf#push-and-pull)** or **[`user:password@host:port/db`](./conf#push-and-pull)** params are mandatory.

| param                                 | required | description                                   |
| :-----------------------------------  | :------- | :-------------------------------------------- |
| driver                                | yes      | driver to use for querying (`mysql2`, `better-sqlite`, `libsql`, `turso`)|
| **[`out`](./conf#migrations-folder)**     | no       | migrations folder, default=`./drizzle`        |
| `uri`                                 | no       | database connection string                    |
...
export default {
  ...
  driver: 'pg',
  dbCredentials: {
    uri: "postgresql://postgres:password@host:port/db",
  }
} satisfies Config;
```
</Tab>
...
```shell
$ drizzle-kit introspect:pg --out=migrations/ --uri=postgresql://user:pass@host:port/db_name
...
Either **[`uri`](./conf#push-and-pull)** or **[`user:password@host:port/db`](./conf#push-and-pull)** params are **required**.

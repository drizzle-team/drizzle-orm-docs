...
### Partial select
`Fields` operator lets you include or omit columns you want to get from the database
...
**Only include fields from nested relations**
<Section>
```typescript copy
...
<Callout type="warning" emoji="⚠️">
  `offset` is only available for top-lvl query
</Callout>
```typescript 
await db.query.posts.findMany({
	limit: 5,

import { ResultSet, Client, Transaction as Transaction$1 } from '@libsql/client';
...
declare class LibSQLSession extends SQLiteSession<'async', ResultSet> {
    private client;
    private logger;
    constructor(client: Client, dialect: SQLiteAsyncDialect, options?: LibSQLSessionOptions);
    prepareQuery<T extends Omit<PreparedQueryConfig, 'run'>>(query: Query, fields?: SelectedFieldsOrdered, tx?: LibSQLTransaction): PreparedQuery<T>;
    batch(queries: SQL[]): Promise<ResultSet[]>;
    transaction(transaction: (tx: LibSQLTransaction) => void | Promise<void>): Promise<void>;
}
declare class LibSQLTransaction extends Transaction<'async', ResultSet> {
    constructor(session: LibSQLSession, 
    /** @internal */
    tx: Promise<Transaction$1>);
}
declare class PreparedQuery<T extends PreparedQueryConfig = PreparedQueryConfig> extends PreparedQuery$1<{
    ...
    private logger;
    private fields;
    constructor(client: Client | Transaction$1, tx: Promise<Transaction$1> | undefined, queryString: string, params: unknown[], logger: Logger, fields: SelectedFieldsOrdered | undefined);
...
export { DrizzleConfig, LibSQLDatabase, LibSQLSession, LibSQLSessionOptions, LibSQLTransaction, PreparedQuery, drizzle };

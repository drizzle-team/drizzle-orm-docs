---
title: DrizzleORM v0.29.3 release
pubDate: 2024-01-02
description: Fixed expo peer dependencies.
---

## Fixes: 

- Make expo peer dependencies optional ([#1714](https://github.com/drizzle-team/drizzle-orm/pull/1714))

For more info you can check [Expo docs](https://expo.dev/) and [Get started with Expo SQLite and Drizzle](https://orm.drizzle.team/docs/get-started-sqlite#expo-sqlite).

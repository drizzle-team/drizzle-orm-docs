...
In this scenario, the user's profile information is stored in a separate table, `profile_info`. Since the foreign key is located in the `profile_info` table, the `users` table does not include any fields or direct references to `profile_info`. This design indicates to TypeScript that `user.profileInfo` can be nullable. The `profileInfoRelations` object defines the relationship from the `profile_info` table back to the `users` table by linking the `userId` field in `profile_info` to the `id` field in `users`:
...
```ts copy {18}
import { relations } from 'drizzle-orm';
import { pgTable, integer, jsonb, text, serial } from 'drizzle-orm/pg-core';

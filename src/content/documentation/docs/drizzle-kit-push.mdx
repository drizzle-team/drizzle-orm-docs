...
### Including tebles, schemas and extensions
...
which creates it's own tables in public schema, so it can ignore them.
...
| [`schema`](/kit-docs/conf#schema-files-paths)                       | required | path to typescript schema file or folder with multiple schema files     |
| driver                                                              |          | driver to use for querying (`aws-data-api`, `d1-http`, `turso`, `expo`) |
| [`tablesFilter`](/kit-docs/conf#multi-project-schema)               |          | table name filter                                                       |
| [`schemaFilter`](/kit-docs/config-reference#schemafilter)           |          | Schema name filter. Default: `["public"]`                               |
| [`extensionsFilters`](/kit-docs/config-reference#extensionsfilters) |          | Database extensions internal database filters                           |
...
| `verbose`                                                           |          | print all queries that will be executed                                 |
| `strict`                                                            |          | always ask for an approve before pushing the schema                     |

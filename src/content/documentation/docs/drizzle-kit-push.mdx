...
<hr/>
<br/>
We recommend configuring `drizzle-kit` through [drizzle.config.ts](/docs/kit-config-file) file, 
...
| [`schema`](/kit-docs/conf#schema-files-paths)                         | `required` | Path to typescript schema file(s) or folder(s) with multiple schema files     |

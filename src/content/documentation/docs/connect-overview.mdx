...
Mostly every driver supports initialization via either **connection URL** or a set of **conneciton params**  
```ts
import { drizzle } from "drizzle-orm"
...
const db = await drizzle("neon-http", process.env.DATABSE_URL);
const db = await drizzle("neon-websocket", process.env.DATABSE_URL);
const db = await drizzle("vercel-postgres");
const db = await drizzle("planetscale", process.env.DATABSE_URL);
...
Just in case if you're not familiar with database connecion URL concept
```
...
<Flex>
  ...
  title='Native SQLite'
  links={[
      ["Expo SQLite", "/docs/get-expo-sqlite"], 
  ...
  title='Others'
  links={[
      ["AWS Data API", "/docs/get-aws-data-api"], 

...
To implement an upsert query in PostgreSQL and SQLite (skip to [MySQL](/learn/guides/upsert#mysql)) with Drizzle you can use `.onConflictDoUpdate()` method:
...
In addition to the custom function, if you need to programatically return any calculations or values on any fields during the upsert, you can follow the example below:

```ts copy {22}
const values = [
  {
    id: 1,
    lastLogin: '2024-03-16T15:44:41.141Z', // this date needs to be updated within the upsert
    active: true,
  },
  // other items
];

const setValues = buildConflictUpdateColumns(users, [
	  'active'
    // any other properties to be included
]);

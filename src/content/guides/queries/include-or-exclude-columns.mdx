...
To include all columns with extra columns you can use `getTableColumns()` function like this:
...
To exclude columns you can use `getTableColumns()` function like this:
  ...
  <CodeTab>
    ...
    await db
      .select({
        postId: posts.id,
        comment: { ...rest },
        user: getTableColumns(users), // equivalent to "user: users"
      })
      .from(posts)

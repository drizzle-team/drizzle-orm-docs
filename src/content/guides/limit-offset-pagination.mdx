...
You can improve perfomance of `limit/offset` pagination by using `deferred join` technique. This method performs the pagination on a subset of the data instead of the entire table.
...
So, if your database experiences frequently insert and delete operations in real time or you need high perfomance to paginate large tables, you should consider using [cursor-based](/learn/guides/cursor-based-pagination) pagination instead.

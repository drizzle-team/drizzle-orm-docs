...
The first step is to install **Drizzle ORM** and `pg` package which we will use as a driver. The second step is to install **Drizzle Kit** and types for `pg`. [Drizzle Kit](/kit-docs/overview) - CLI companion for automatic SQL migrations generation and rapid prototyping. 

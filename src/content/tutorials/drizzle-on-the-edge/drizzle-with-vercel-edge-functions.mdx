...
- [Neon serverless driver](https://neon.tech/docs/serverless/serverless-driver) allows you access any `PostgreSQL` client and query data from serverless and edge environments over HTTP or WebSockets in place of TCP.
- [Vercel Postgres client](https://vercel.com/docs/storage/vercel-postgres) is built on top of the `Neon serverless driver` and allows you access any `PostgreSQL` client. However, it implements specific connection string checks that may not be compatible with non-Vercel Postgres databases. That's why we recommend to use the `Neon serverless driver` for non-Vercel Postgres databases.
- [PlanetScale serverless driver](https://planetscale.com/docs/tutorials/planetscale-serverless-driver) allows you access any `MySQL` client and execute queries over an HTTP connection, which is generally not blocked by cloud providers.
- [libSQL client](https://docs.turso.tech/sdk/ts/reference) allows you to access [Turso](https://docs.turso.tech/introduction) database.
...
Run the `drizzle-kit push:<dialect>` command to push your changes to the database:
...
Create a new project in the [dashboard](https://vercel.com/new/mykhailos-projects-58970afa) or run the `vercel` command to deploy your project:

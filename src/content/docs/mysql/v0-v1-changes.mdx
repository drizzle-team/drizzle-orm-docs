...
RQBv1 has been removed. Use [Relational Queries v2](/docs/mysql/relations-v2) with the new `defineRelations()` API instead and new features. You can find more details about migrating from rqb v1 to v2 [here](/docs/mysql/relations-v1-v2)
...
The `--strict` flag has been removed — its behavior is now the default. `drizzle-kit push` always prompts for confirmation for data-loss statements unless the `--force` flag is passed. Use [drizzle-kit push --explain](/docs/mysql/drizzle-kit-push) to preview the SQL that would be executed before running it.

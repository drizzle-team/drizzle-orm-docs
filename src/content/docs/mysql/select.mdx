...
<Callout type='warning'>
`getColumns` available starting from `drizzle-orm@1.0.0-beta.2`(read more [here](/docs/mysql/upgrade-v1))
...
<Callout type="info">
`cast(... as int)` is necessary because `count()` returns `decimal` in MySQL, which is treated as a string value instead of a number.

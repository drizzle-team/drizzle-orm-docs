...
- Get familiar with [One-to-many Relation](/docs/mysql/relations#one-to-many)
- Get familiar with [Drizzle Seed](/docs/mysql/seed-overview)
</Prerequisites>
...
export const users = mysqlTable('users', {
	id: int().primaryKey().autoincrement(),
	name: text(),
    reportsTo: int('reports_to').references((): AnyMySqlColumn => users.id),
});
```
</CodeTab>
</CodeTabs>

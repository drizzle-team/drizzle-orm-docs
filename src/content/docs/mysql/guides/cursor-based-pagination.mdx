...
- [Select statement](/docs/mysql/select) with [order by clause](/docs/mysql/select#order-by)
- [Relational queries](/docs/mysql/rqb) with [order by clause](/docs/mysql/rqb#order-by)
- [Indices](/docs/mysql/indexes-constraints)
</Prerequisites>
...
<Section>
```ts copy {7,8}
import { index, ...imports } from 'drizzle-orm/mysql-core';
...
So, if you need to directly navigate to a specific page or you need simpler implementation of pagination, you should consider using [offset/limit](/docs/mysql/guides/limit-offset-pagination) pagination instead.

...
<CodeTabs items={["index.ts", "schema.ts"]}>
    ...
    export const posts = t.mssqlTable("posts", {
      id: t.int().primaryKey().identity(),
      content: t.text().notNull(),
      authorId: t.int("author_id").notNull(),
    });
    
    export const users = t.mssqlTable("users", {
      id: t.int().primaryKey().identity(),
      name: t.text().notNull(),
    });
...
```ts
// schema.ts
import { int, text, mssqlTable, text, datetime2 } from "drizzle-orm/mssql-core";
...
export const users = mssqlTable("users", {
  ...
  lastLogin: datetime2("last_login"),
  subscriptionEnd: datetime2("subscription_end"),
  lastActivity: datetime2("last_activity"),
  preferences: text("preferences"),        // JSON column for user settings/preferences
...
### Prepared statements
Prepared statements are designed to massively improve query performance — [see here.](/docs/mssql/perf-queries)

...
```typescript
async function selectUsers(withName: bit) {
  return db
    .select({
      id: users.id,
...
<Callout type='warning'>
`getColumns` available starting from `drizzle-orm@1.0.0-beta.2`(read more [here](/docs/mssql/upgrade-v1))
...
```sql
select [id], [name], [age] from [users] where [id] = @p1; -- params: [42]
```
</Callout>
...
MSSQL doesn't support native `RETURNING`, so `insert`, `update`, and `delete` statements that depend on returning rows are not available inside `with` for this dialect.
...
<Callout type="info">
`cast(... as int)` is necessary because `count()` returns `decimal` in MSSQL, which is treated as a string value instead of a number.
Alternatively, you can use [`.mapWith(Number)`](/docs/mssql/sql#sqlmapwith) to cast the value to a number at runtime.
...
The `IGNORE INDEX` hint tells the optimizer to avoid using specific indexes for the query. MSSQL will consider all other indexes (if any) or perform a full table scan if necessary.
...
The `FORCE INDEX` hint forces the optimizer to use the specified index(es) for the query. If the specified index cannot be used, MSSQL will not fall back to other indexes; it might resort to a full table scan instead.

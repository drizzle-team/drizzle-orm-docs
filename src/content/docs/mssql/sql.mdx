...
```sql
select * from [users] where [users].[id] = @p1; --> [69]
```
...
--> vs
select * from users where id = @p1; --> [12]
```
</Section>
...
```
```sql
select * from users where id = @p1 or id = @p2 or id = @p3 or id = @p4 or id = @p5; --> [0, 1, 2, 3, 4]
```
</Section>
...
```
```sql
select * from [users] where [users].[id] = @p1; --> [ 12 ]
```
</Section>
...
Here you can see a usage for **[`sql<T>`](/docs/mssql/sql#sqlt)**, **[`sql``.mapWith()`](/docs/mssql/sql#sqlmapwith)**, **[`sql``.as<T>()`](/docs/mssql/sql#sqlast)**.
...
```
```sql
select * from [users] where [users].[id] = @p1; --> [ 77 ]
```
</Section>
...
```
```sql
select * from [users] where lower([users].[name]) like lower(@p1); --> [ "%Ale%" ]
```
</Section>

...
- [Filtering](/docs/mssql/select#filters) and [Filter operators](/docs/mssql/operators);
</Prerequisites>
...
```sql
select * from [posts] where len([posts].[title]) < 8;
select * from [posts] where ((len([posts].[title]) < 8) and ([posts].[views] > 200));
```
</Section>

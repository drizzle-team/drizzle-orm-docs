...
<CodeTab>
```ts copy {10}
import { int, mssqlTable, real } from "drizzle-orm/mssql-core";

export const users = mssqlTable("users", {
    id: int().identity().primaryKey(),
});

export const bloodPressure = mssqlTable("bloodPressure", {
	bloodPressureId: int().identity().primaryKey(),
	pressure: real(),
...
```
- You can [refine](/docs/mssql/guides/seeding-with-partially-exposed-schema#refining-the-userid-column-generator) the `userId` column generator;
...
- Otherwise, you can [refine](/docs/mssql/guides/seeding-with-partially-exposed-schema#refining-the-userid-column-generator) the `userId` column generator. 

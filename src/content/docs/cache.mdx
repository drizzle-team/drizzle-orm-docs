...
By default, Drizzle sends every query straight to your database with no hidden actions or caching - you can always see exactly 
what runs. If you need caching, just enable the `cache` option to control when to read from or 
write to `cache` and how entries are invalidated after updates. This runs like a `beforeQuery` 
hook before each query executes. Drizzle also includes several built-in caching strategies you can use out of the box.

To enable caching, add `cache` callbacks to your Drizzle setup or pick one of the built-in options. 
For example, you can use the `upstashCache()` helper we created in collaboration with the Upstash team.
...
// disable auto-invalidation for this query, if you are fine with eventual consistency for this specific query

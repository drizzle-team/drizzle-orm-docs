...
- [Get typed table columns](/docs/singlestore/goodies#get-typed-columns)
- [Joins](/docs/singlestore/joins)
- [Relational queries](/docs/singlestore/rqb)
- [Partial select with relational queries](/docs/singlestore/rqb#partial-fields-select)
</Prerequisites>

<Callout>
SingleStore does not support foreign key constraints, so the schemas below use plain columns
(no `.references()`). Relations for the relational queries API are declared with
`defineRelations` and describe the logical relationships used at query time.
</Callout>

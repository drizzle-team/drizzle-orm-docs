...
- [Select statement](/docs/singlestore/select) with [order by clause](/docs/singlestore/select#order-by) and [limit & offset clauses](/docs/singlestore/select#limit--offset)
- [Relational queries](/docs/singlestore/rqb) with [order by clause](/docs/singlestore/rqb#order-by) and [limit & offset clauses](/docs/singlestore/rqb#limit--offset)
- [Dynamic query building](/docs/singlestore/dynamic-query-building)
</Prerequisites>
...
<CodeTabs items={["index.ts", "schema.ts"]}>
  <CodeTab>
    ...
    ```sql
    select * from `users` order by `users`.`id` asc limit 4 offset 4;
    ```
  ...
  <CodeTab>
    ...
    +----+------------+-----------+---------------------+
    |  7 | Clara      | Wilson    | 2024-03-08 13:22:55 |
    +----+------------+-----------+---------------------+
    |  8 | David      | Moore     | 2024-03-08 13:34:55 |
    +----+------------+-----------+---------------------+
    ```
  </CodeTab>
</CodeTabs>
...
import { SingleStoreColumn, type SingleStoreSelect } from 'drizzle-orm/singlestore-core';

function withPagination<T extends SingleStoreSelect>(
  qb: T,
  orderByColumn: SingleStoreColumn | SQL | SQL.Aliased,
  page = 1,
  pageSize = 3,
) {
  return qb
...
So, if your database experiences frequently insert and delete operations in real time or you need high performance to paginate large tables, you should consider using [cursor-based](/docs/singlestore/guides/cursor-based-pagination) pagination instead.

...
Drizzle Kit automatically picks an available SingleStore driver from your current project based on `dialect: "singlestore"`.
SingleStore uses MySQL-compatible connection parameters, so you usually do not need a separate `driver` option.
...
### `tablesFilter`
<Callout>
If you want to run multiple projects with one database - check out [our guide](/docs/singlestore/goodies#multi-project-schema).
</Callout>
<rem025/>

...
Relational queries are an extension to Drizzle's original **[query builder](/docs/singlestore/select)**.
...
<Callout type="info" emoji="ℹ️">
	`drizzle` import path depends on the **[database driver](/docs/singlestore/connect-overview)** you're using.
</Callout>
...
For any nested `with` queries Drizzle will infer types using [Core Type API](/docs/singlestore/goodies#type-api).
...
relational queries API lets you define filters and conditions with the list of our **[`operators`](/docs/singlestore/operators)**.  
...
export const users = singlestoreTable("users", {
  ...
  preferences: json("preferences"),        // JSON column for user settings/preferences
...
You can use same ordering **[core API](/docs/singlestore/select#order-by)** or use
...
<Callout type="warning" emoji="⚠️">
	As of now aggregations are not supported in `extras`, please use **[`core queries`](/docs/singlestore/select)** for that.
</Callout>
...
### Prepared statements
Prepared statements are designed to massively improve query performance — [see here.](/docs/singlestore/perf-queries)

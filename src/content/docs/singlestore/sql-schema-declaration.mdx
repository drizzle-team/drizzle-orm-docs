...
about the `drizzle.config.ts` file and migrations with Drizzle, please check: [link](/docs/singlestore/drizzle-config-file)
```ts
...
- Schemas, which are equivalent to databases in SingleStore
- Enums
- Views
- etc.
...
there is no such thing as a common table object in Drizzle. You need to choose the SingleStore table object.
...
<Callout>
This example uses a db object, whose initialization is not covered in this part of the documentation. To learn how to connect to the database, please refer to the [Connections Docs](/docs/singlestore/get-started-singlestore)
</Callout>
...
\
In SingleStore, there is an entity called `Schema`, but in SingleStore terms, this is equivalent to a `Database`. 
...
<Flex>
  <LinksList 
    title='Manage schema'
    links={[
        ["Column types", "/docs/singlestore/column-types"], 
        ["Indexes and Constraints", "/docs/singlestore/indexes-constraints"],
        ["Database Schemas", "/docs/singlestore/schemas"],
      ]}
  />
  <LinksList 
    title='Zero to Hero'
    links={[
        ["Database connection", "/docs/singlestore/connect-overview"], 
        ["Data querying", "/docs/singlestore/data-querying"], 
        ["Migrations", "/docs/singlestore/migrations"], 
      ]}
  />
</Flex>

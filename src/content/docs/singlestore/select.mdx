...
and whatever it doesn't support yet, can be added by the user with the powerful [`sql`](/docs/singlestore/sql) operator.
...
If you need to apply runtime transformations to the returned value, you can use the [`.mapWith()`](/docs/singlestore/sql#sqlmapwith) method.
</Callout>
...
Sneak peek of advanced partial select, for more detailed advanced usage examples - see our [dedicated guide](/docs/singlestore/guides/include-or-exclude-columns).

<Callout type='warning'>
`getColumns` available starting from `drizzle-orm@1.0.0-beta.2`(read more [here](/docs/singlestore/upgrade-v1))
...
You can filter the query results using the [filter operators](/docs/singlestore/operators) in the `.where()` method:
...
All filter operators are implemented using the [`sql`](/docs/singlestore/sql) function.
...
Sneak peek of conditional filtering, for more detailed advanced usage examples - see our [dedicated guide](/docs/singlestore/guides/conditional-filters-in-query).
...
usage examples - see our dedicated [limit offset pagination](/docs/singlestore/guides/limit-offset-pagination)
and [cursor pagination](/docs/singlestore/guides/cursor-based-pagination) guides.
...
<Callout type="info">
`cast(... as int)` is necessary because `count()` returns `decimal` in SingleStore, which is treated as a string value instead of a number.
Alternatively, you can use [`.mapWith(Number)`](/docs/singlestore/sql#sqlmapwith) to cast the value to a number at runtime.

If you need count aggregation - we recommend using our [`$count`](/docs/singlestore/select#count) API
</Callout>

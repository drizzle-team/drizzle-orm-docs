...
- Drizzle Kit [overview](/docs/singlestore/kit-overview) and [config file](/docs/singlestore/drizzle-config-file)
</Prerequisites>
...
It's designed to cover [code first](/docs/singlestore/migrations) approach of managing Drizzle migrations. 
You can apply generated migrations using [`drizzle-kit migrate`](/docs/singlestore/drizzle-kit-migrate), using drizzle-orm's `migrate()`, 
...
you can set them either via [drizzle.config.ts](/docs/singlestore/drizzle-config-file) config file or via CLI options
...
for DDL alternations currently not supported by Drizzle Kit or data seeding. Extended docs on custom migrations - [see here](/docs/singlestore/kit-custom-migrations)
...
### Extended example
Example of how to create a custom SingleStore migration file named `0001_seed-users.sql` 

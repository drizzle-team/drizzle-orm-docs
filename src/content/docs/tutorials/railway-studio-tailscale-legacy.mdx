---
title: "Drizzle Studio and Gateway on Railway behind Tailscale (legacy)"
date: "2026-08-25"
---
...
Drizzle now has Railway templates that put the browser on your tailnet with no forwarder to configure. You click one sign-in link in the deploy log, and the log prints the address to open. To start from scratch, follow [Drizzle Studio and Gateway on Railway with Tailscale](/docs/tutorials/railway-studio-tailscale). If you already run Drizzle Studio or Gateway on Railway, follow [Add Tailscale to Drizzle Studio or Gateway on Railway](/docs/tutorials/railway-tailscale-existing-project).
</Callout>
...
If you want the same private access for `drizzle-kit` and other database clients, see [Connect to a private Railway database with Tailscale](/docs/tutorials/railway-postgres-tailscale-legacy).

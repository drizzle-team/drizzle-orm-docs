...
- You should have installed the [postgres](https://www.npmjs.com/package/postgres) package. 
<Npm>
  postgres
</Npm>
...
<Callout type="warning">
You might see a red underline under the imports if you're using VS Code and answered `Yes` on using settings for `Deno` and `Edge Functions`. The Edge Function will still execute.
</Callout>

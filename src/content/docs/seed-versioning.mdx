...
|     |     api version  |   npm version    |     changes                             |
| :-: | :--------------: | :--------------: | :-------------:                         |
|     |  `v1`            | `0.1.1`          |                                         |
...
**You will be affected by changes in unique `interval` generator**

If your table includes a unique column of type `interval`:
...
If you use the unique `interval` generator in your seeding script, as shown in the script below:
...
- Both non-unique and unique `string` generators were changed
...
**You will be affected by changes in the `string` generator**

If your table includes a column of a text-like type with a maximum length parameter or a unique column of a text-like type:
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
<Tab>
```ts
import { drizzle } from "drizzle-orm/node-postgres";
...
If you use the `string` generator in your seeding script, as shown in the script below:
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite']}>
<Tab>
```ts
import { drizzle } from "drizzle-orm/node-postgres";

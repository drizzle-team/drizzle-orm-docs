...
export default defineConfig({
  ...
  provider: "supabase", // lets the Supabase CLI apply the generated migrations
  dialect: "postgresql",
});
```

<Callout type="info">
Because this tutorial applies migrations with the Supabase CLI, the config uses the
`provider: "supabase"` option. With it, `drizzle-kit generate` duplicates each generated migration
`.sql` file to the root of the `supabase/migrations` folder, so the Supabase CLI can read and apply
them. Read more about the [`provider` option](/docs/drizzle-config-file#provider).
</Callout>
...
If you are using Visual Studio Code, follow the [Supabase documentation](https://supabase.com/docs/guides/functions/development-environment) to setup settings for Deno.
...
{
  "imports": {
    "drizzle-orm/": "npm:/drizzle-orm@rc/",
...
```typescript copy filename="supabase/functions/drizzle-tutorial/index.ts" {2,4,14,17-18,20-24}
// Setup type definitions for built-in Supabase Runtime APIs
...
Read more about Connection Pooler in the [documentation](https://supabase.com/docs/guides/database/connecting-to-postgres#poolers).
...
To apply migrations to the remote database using the [drizzle-kit migrate](/docs/drizzle-kit-migrate) command, provide `dbCredentials` in your Drizzle config file. Use the connection string from the previous step:

```typescript copy filename="drizzle.config.ts" {6,9}
import { defineConfig } from "drizzle-kit";
...
Make sure the `DATABASE_URL` environment variable is available in your local environment and run the following command:

```bash copy
npx drizzle-kit migrate
```

Alternatively, you can apply migrations using the [supabase db push](https://supabase.com/docs/reference/cli/supabase-db-push) command:

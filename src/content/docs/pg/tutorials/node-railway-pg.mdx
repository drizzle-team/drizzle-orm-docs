...
Log in to your [Railway dashboard](https://railway.app/dashboard) and click the `New` button.

![](@/assets/images/tutorials/railway-create-service-menu.png)

#### Provision a PostgreSQL database

On the project canvas, click the `New` button in the top right corner and select `Database` → `PostgreSQL`. You can also search for PostgreSQL using the search bar. Railway will provision a new PostgreSQL instance for you.
...
The `generate` command only creates migration SQL files based on your schema — it does **not** apply any changes to the database. These migrations are stored in the `./migrations` directory, as specified in your `drizzle.config.ts`. This directory will contain a folder for each migration, each holding a `migration.sql` file with the statements necessary to update your database schema and a `snapshot.json` file storing a snapshot of the schema at that migration stage.
...
Alternatively, you can push changes directly to the database using [Drizzle kit push command](/docs/drizzle-kit-push):
...
📦 <project root>
 ...
 │  └ 📜 index.ts
 ├ 📂 migrations
 │  └ 📂 20242409125510_optimal_the_fury
 │    ├ 📜 migration.sql
 │    └ 📜 snapshot.json
 ├ 📜 .env
 ├ 📜 drizzle.config.ts
...
In your Railway project, click the `Add` button in the top right corner of the canvas and select `GitHub Repo`. Connect your GitHub account if prompted, then select the repository you just pushed.
...
Click `New Variable`, set the name to `DATABASE_URL`, and use the Railway variable reference `${{Postgres.DATABASE_URL}}` as the value.
...
You can deploy [Drizzle Studio](/drizzle-studio/overview) alongside your application on Railway to browse and manage your database directly from the browser. You can use the [Drizzle Studio Railway template](https://railway.com/deploy/drizzle-studio-1) or follow the steps below.

<Steps>
#### Add a new service from a template

In your Railway project, click the `Add` button in the top right corner of the canvas and select `Template`.

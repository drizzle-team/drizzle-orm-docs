...
Navigate to [Dashboard](https://supabase.com/dashboard/project/_) and choose your project. Click `Connect` on the top of the page and copy the URI from the `Direct connection string` section. Remember to replace the password placeholder with your actual database password.
...
Read more about Connection modes in the [documentation](https://supabase.com/docs/guides/database/connecting-to-postgres#connection-pooler).
...
<Callout type="info">
If you want to apply migrations using the Supabase CLI, add the `provider: "supabase"` option. With
it, `drizzle-kit generate` duplicates each generated migration `.sql` file to the root of your
migrations folder, so the Supabase CLI can read and apply them. Read more about the
[`provider` option](/docs/drizzle-config-file#provider).
</Callout>

```typescript copy filename="drizzle.config.ts" {10}
import { config } from 'dotenv';
...
export default defineConfig({
  ...
  dialect: 'postgresql',
  provider: 'supabase', // optional — lets the Supabase CLI apply the generated migrations
  dbCredentials: {
...
📦 <project root>
 ...
 ├ 📂 supabase
 │   ├ 📂 migrations
 │   │  └ 📂 20260708132309_modern_gravity
 │   │     ├ 📜 migration.sql
 │   │     └ 📜 snapshot.json
 │   │ 📜 20260708132309_modern_gravity.sql
 │   └ 📜 config.toml
 ├ 📜 .env
 ├ 📜 drizzle.config.ts
 ├ 📜 package.json

...
successfully applied migrations in your database in log table named `__drizzle_migrations` in `public` schema.
...
Prints the planned SQL changes with an explanation, without applying them (dry run) when running the `drizzle-kit push` command

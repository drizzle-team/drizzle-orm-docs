...
```sql
select "id", "name", "location", ST_Distance("location", ST_SetSRID(ST_MakePoint(-73.935242, 40.73061), 4326)) 
from "stores" 
order by "stores"."location" <-> ST_SetSRID(ST_MakePoint(-73.935242, 40.73061), 4326) 
limit 1;
```
</Section>
...
```sql
select "id", "name", "location" from "stores" where ST_Within("stores"."location", ST_MakeEnvelope(-88, 40, -73, 43, 4326))
```
</Section>

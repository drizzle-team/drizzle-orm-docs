...
- [Relational queries](/docs/rqb-v2) with [order by clause](/docs/rqb-v2#order-by)
...
(t) => [
  ...
  index('first_name_and_id_index').on(t.firstName.asc(), t.id.asc()),
]);
```

```sql
CREATE INDEX IF "first_name_index" ON "users" ("first_name" ASC);
CREATE INDEX IF "first_name_and_id_index" ON "users" ("first_name" ASC,"id" ASC);
```
</Section>

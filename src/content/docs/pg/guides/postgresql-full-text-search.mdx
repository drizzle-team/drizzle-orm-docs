...
select "id", "title" from "posts" 
  where to_tsvector('english', "posts"."title") @@ plainto_tsquery('english', 'discover Italy')
```
...
```sql
select "id", "title" from "posts" 
  where to_tsvector('english', "posts"."title") @@ phraseto_tsquery('english', 'family trip')
```
...
```sql
select "id", "title" from "posts" 
  where to_tsvector('english', "posts"."title") @@ websearch_to_tsquery('english', 'family or first trip Europe or Asia')
```
...
<CodeTabs items={["schema.ts", "migration.sql", "db_data"]}>
  ...
  CREATE INDEX "search_index" ON "posts" USING gin ((

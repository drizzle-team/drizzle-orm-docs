...
```sql
select "id", "title", "body", "body_search" from "posts" where "posts"."body_search" @@ to_tsquery('english', 'bring')
```
</Section>
...
```sql
select "id", "title", "body", "search" from "posts" where "posts"."search" @@ to_tsquery('english', 'travel')
```
</Section>

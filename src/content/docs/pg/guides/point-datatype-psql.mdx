...
```sql
select "id", "name", "location"::text, round((location <-> point(-73.935242, 40.73061))::numeric, 2) 
from "stores" 
order by location <-> point(-73.935242, 40.73061) 
limit 1
```
</Section>
...
```sql
select "id", "name", "location"::text from "stores" where "stores"."location" <@ box(point(-88, 40), point(-73, 43));
```
</Section>

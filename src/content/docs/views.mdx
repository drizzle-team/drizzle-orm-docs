...
<Tabs items={['PostgreSQL', 'MySQL', 'SQLite', 'MSSQL', 'CockroachDB']}>
  ...
  <Tab>
    <Section>
      ...
      export const customerView = sqliteView("customers_view").as(qb.select().from(user).where(eq(user.role, "customer")));
      ```
      ```sql
      CREATE VIEW "user_view" AS SELECT * FROM "user";
      CREATE VIEW "customers_view" AS SELECT * FROM "user" WHERE "role" = 'customer';
  ...
  <Tab>
    <Section>
      ...
      export const customerView = mssqlView("customers_view").as(qb.select().from(user).where(eq(user.role, "customer")));
      ```
      ```sql
      CREATE VIEW [user_view] AS (SELECT * FROM "user");
      CREATE VIEW [customers_view] AS (SELECT * FROM "user" WHERE "role" = 'customer');

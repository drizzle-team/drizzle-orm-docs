...
const res = await db.query.users.findMany({
	extras: {
		fullName: sql<string>`concat(${users.name}, " ", ${users.name})`,
	},
	with: {
		usersToGroups: {
			with: {
...
const res = await db.query.posts.findMany({
	extras: (table, { sql }) => ({
		contentLength: (sql<number>`length(${table.content})`),
	}),
	with: {
		comments: {
			extras: {
				commentSize: sql<number>`length(${comments.content})`,
			},
		},
	},
});
...
await db.query.users.findMany({
  ...
  extras: {
    totalPostsCount: (table) => db.$count(posts, eq(posts.authorId, t.id)),
  }
});
```
```sql

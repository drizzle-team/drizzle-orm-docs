...
```typescript
import { cockroachTable, serial, text } from 'drizzle-orm/cockroach-core';
...
<Callout type='warning'>
`distinct on` clause is only supported in CockroachDB.
</Callout>
<Section>
```ts
...
<Callout type='warning'>
`getColumns` available starting from `drizzle-orm@1.0.0-beta.2`(read more [here](/docs/cockroach/upgrade-v1))
...
<Callout type="info">
`cast(... as int)` is necessary because `count()` returns `bigint` in CockroachDB and `decimal` in MySQL, which are treated as string values instead of numbers.

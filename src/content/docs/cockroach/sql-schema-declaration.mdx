...
```ts
import { integer, cockroachTable, varchar } from "drizzle-orm/cockroach-core";
...
- Schemas(CockroachDB only)
- Enums
- Sequences(CockroachDB only)
- Views
- Materialized Views
- etc.
...
there is no such thing as a common table object in drizzle. You need to choose the CockroachDB table object for CockroachDB schemas.

![](@/assets/images/table-structure.svg)

```ts copy
import { cockroachTable, integer } from "drizzle-orm/cockroach-core"
...
\
In CockroachDB, there is an entity called a `schema` (which we believe should be called `folders`). This creates a structure in CockroachDB:

![](@/assets/images/postgresql-db-structure.png)

You can manage your CockroachDB schemas with `cockroachSchema` and place any other models inside it.
...
```ts {5-7}
import { integer, cockroachSchema } from "drizzle-orm/cockroach-core";
...
```ts copy
import { AnyPgColumn } from "drizzle-orm/cockroach-core";
...
<Flex>
  <LinksList 
    title='Manage schema'
    links={[
        ["Column types", "/docs/cockroach/column-types"], 
        ["Indexes and Constraints", "/docs/cockroach/indexes-constraints"],
        ["Database Views", "/docs/cockroach/views"],
        ["Database Schemas", "/docs/cockroach/schemas"],
        ["Sequences", "/docs/cockroach/sequences"],
      ]}
  />
  <LinksList 
    title='Zero to Hero'
    links={[
        ["Database connection", "/docs/cockroach/connect-overview"], 
        ["Data querying", "/docs/cockroach/data-querying"], 
        ["Migrations", "/docs/cockroach/migrations"], 
      ]}
  />
</Flex>

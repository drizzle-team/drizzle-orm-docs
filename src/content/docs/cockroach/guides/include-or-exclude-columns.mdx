...
- [Get typed table columns](/docs/cockroach/goodies#get-typed-columns)
- [Joins](/docs/cockroach/joins)
- [Relational queries](/docs/cockroach/rqb)
- [Partial select with relational queries](/docs/cockroach/rqb#partial-fields-select)
</Prerequisites>

<Callout>
CockroachDB uses [Relational Queries v1](/docs/cockroach/rqb): relations are declared with the
...
<CodeTabs items={["index.ts", "schema.ts"]}>
  ...
  export const comments = cockroachTable('comments', {
    id: int4().primaryKey().generatedAlwaysAsIdentity(),
    postId: int4('post_id').notNull().references(() => posts.id),
    userId: int4('user_id').notNull().references(() => users.id),
    content: text().notNull(),

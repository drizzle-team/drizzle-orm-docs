...
To set current timestamp as a default value in CockroachDB, you can use the `defaultNow()` method or `sql` operator with `now()` function which returns the current date and time with the time zone:

<Section>
```ts copy {6,9}
import { sql } from 'drizzle-orm';
import { timestamp, cockroachTable, int4 } from 'drizzle-orm/cockroach-core';
...
To set unix timestamp as a default value in CockroachDB, you can use the `sql` operator and `extract(epoch from now())` function which returns the number of seconds since `1970-01-01 00:00:00 UTC`:
...
CREATE TABLE "users" (
	"id" int4 PRIMARY KEY GENERATED ALWAYS AS IDENTITY (INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1 CACHE 1),
	"timestamp" int4 DEFAULT extract(epoch from now()) NOT NULL
);
```

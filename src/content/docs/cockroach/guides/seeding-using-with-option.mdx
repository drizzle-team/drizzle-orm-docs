...
- Get familiar with [One-to-many Relation](/docs/cockroach/relations#one-to-many)
...
export const posts = cockroachTable('posts', {
	id: int4().primaryKey().generatedAlwaysAsIdentity(),
	content: text(),
	authorId: int4('author_id').notNull().references(() => users.id),
});
```
</CodeTab>
</CodeTabs>
...
export const users = cockroachTable('users', {
	id: int4().primaryKey().generatedAlwaysAsIdentity(),
	name: text(),
    reportsTo: int4('reports_to').references((): AnyCockroachColumn => users.id),
});
```
</CodeTab>
</CodeTabs>

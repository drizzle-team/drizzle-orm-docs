...
- [Select statement](/docs/cockroach/select) with [order by clause](/docs/cockroach/select#order-by) and [limit & offset clauses](/docs/cockroach/select#limit--offset)
- [Relational queries](/docs/cockroach/rqb) with [order by clause](/docs/cockroach/rqb#order-by) and [limit & offset clauses](/docs/cockroach/rqb#limit--offset)
- [Dynamic query building](/docs/cockroach/dynamic-query-building)
</Prerequisites>
...
Drizzle has useful relational queries API, that lets you easily implement `limit/offset` pagination. CockroachDB uses [Relational Queries v1](/docs/cockroach/rqb), accessed via `db._query`:
...
import { CockroachColumn, type CockroachSelect } from 'drizzle-orm/cockroach-core';

function withPagination<T extends CockroachSelect>(
  qb: T,
  orderByColumn: CockroachColumn | SQL | SQL.Aliased,
  page = 1,
  pageSize = 3,
) {
  return qb
...
So, if your database experiences frequently insert and delete operations in real time or you need high performance to paginate large tables, you should consider using [cursor-based](/docs/cockroach/guides/cursor-based-pagination) pagination instead.

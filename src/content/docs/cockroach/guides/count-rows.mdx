...
- [Filters](/docs/cockroach/operators) and [sql operator](/docs/cockroach/sql)
- [Aggregations](/docs/cockroach/select#aggregations) and [Aggregation helpers](/docs/cockroach/select#aggregations-helpers)
- [Joins](/docs/cockroach/joins)
</Prerequisites>
...
Drizzle has simple and flexible API, which lets you create your custom solutions. In CockroachDB `count()` function returns bigint, which is interpreted as string by the driver, so it should be casted to integer:
...
<CodeTabs items={["index.ts", "schema.ts"]}>
  ...
  export const cities = cockroachTable('cities', {
    id: int4().primaryKey().generatedAlwaysAsIdentity(),
    name: text().notNull(),
    countryId: int4('country_id').notNull().references(() => countries.id),
  });
  ```
</CodeTabs>

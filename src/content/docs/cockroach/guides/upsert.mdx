...
<Callout>
The examples below insert explicit `id` values to trigger a conflict, so the identity column uses
`generatedByDefaultAsIdentity()`. A column defined with `generatedAlwaysAsIdentity()` rejects
explicit values (`cannot insert into column "id"`), which would make an id-based upsert impossible.
</Callout>

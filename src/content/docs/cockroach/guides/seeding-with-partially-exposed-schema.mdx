...
- Get familiar with [Drizzle Seed](/docs/cockroach/seed-overview)
</Prerequisites>
...
<CodeTab>
```ts copy {10}
import { doublePrecision, int4, cockroachTable } from "drizzle-orm/cockroach-core";

export const users = cockroachTable("users", {
    id: int4().primaryKey().generatedAlwaysAsIdentity(),
});

export const bloodPressure = cockroachTable("bloodPressure", {
	bloodPressureId: int4().primaryKey().generatedAlwaysAsIdentity(),
	pressure: doublePrecision(),
	userId: int4().references(() => users.id).notNull(),
})
```
</CodeTab>
</CodeTabs>
...
```
- You can [refine](/docs/cockroach/guides/seeding-with-partially-exposed-schema#refining-the-userid-column-generator) the `userId` column generator;
...
- Otherwise, you can [refine](/docs/cockroach/guides/seeding-with-partially-exposed-schema#refining-the-userid-column-generator) the `userId` column generator. 

...
```ts
import { serial, text, cockroachTableCreator } from 'drizzle-orm/cockroach-core';
...
`getColumns` is available starting from `drizzle-orm@1.0.0-beta.2`. For pre-1 versions, use `getTableColumns`.
</Callout>

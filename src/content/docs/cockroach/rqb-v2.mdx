...
```typescript copy
import { type AnyPgColumn, boolean, integer, cockroachTable, primaryKey, text, timestamp } from 'drizzle-orm/cockroach-core';
...
```ts
// schema.ts
import { integer, jsonb, cockroachTable, text, timestamp } from "drizzle-orm/cockroach-core";

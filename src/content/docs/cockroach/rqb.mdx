...
<CodeTabs items={["index.ts", "schema.ts"]}>
	...
	```typescript copy
	import { integer, serial, text, cockroachTable } from 'drizzle-orm/cockroach-core';
...
## Modes
Drizzle relational queries for CockroachDB use the default relational query mode. You do not need to configure a database-specific mode.
...
```typescript copy
	import { type AnyPgColumn, boolean, integer, cockroachTable, primaryKey, serial, text, timestamp } from 'drizzle-orm/cockroach-core';

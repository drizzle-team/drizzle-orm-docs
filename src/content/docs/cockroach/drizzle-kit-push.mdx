...
export default defineConfig({
  ...
  dbCredentials: {
    url: "cockroach://user:password@host:port/dbname",
  },
});
```
```shell
...
```shell
npx drizzle-kit push --dialect=cockroach --schema=./src/schema.ts --url=cockroach://user:password@host:port/dbname
```
...
<Npx>
drizzle-kit push dialect=cockroach schema=src/schema.ts url=cockroach://user:password@host:port/dbname
drizzle-kit push dialect=cockroach schema=src/schema.ts driver=pglite url=database/
drizzle-kit push dialect=cockroach schema=src/schema.ts --tablesFilter='user*' --extensionsFilters=postgis url=cockroach://user:password@host:port/dbname
</Npx>

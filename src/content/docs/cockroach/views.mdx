...
<Section>
      ...
      import { cockroachTable, cockroachView, serial, text, timestamp } from "drizzle-orm/cockroach-core";
...
<Section>
      ...
      import { cockroachTable, cockroachView, serial, text, timestamp, QueryBuilder} from "drizzle-orm/cockroach-core";
...
// materialized view
const newYorkers = cockroachMaterializedView('new_yorkers', {
  id: serial('id').primaryKey(),
  name: text('name').notNull(),
  cityId: integer('city_id').notNull(),
}).as(sql`select * from ${users} where ${eq(users.cityId, 1)}`);
...
According to the official docs, CockroachDB and CockroachDB have both **[`regular`](https://www.cockroach.org/docs/current/sql-createview.html)**
and **[`materialized`](https://www.cockroach.org/docs/current/sql-creatematerializedview.html)** views.
  
Materialized views in CockroachDB and CockroachDB use the rule system like views do, but persist the results in a table-like form.

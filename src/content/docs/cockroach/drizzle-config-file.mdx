...
export default defineConfig({
  dialect: "cockroach", // "mysql" | "sqlite" | "cockroach" | "turso" | "singlestore" | "mssql"
  schema: "./src/schema/*",
  out: "./drizzle",
});
```
...
successfully applied migrations in your database in log table named `__drizzle_migrations` in `public` schema(CockroachDB only).
...
export default defineConfig({
  ...
  migrations: {
    ...
    schema: 'public', // used in CockroachDB only, `drizzle` by default
  },
});
```

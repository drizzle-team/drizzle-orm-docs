...
## returning
You can insert a row and get it back in SQLite like such:
```typescript copy
...
<Callout>
  Check how to use WITH statement with [select](/docs/sqlite/select#with-clause), [update](/docs/sqlite/update#with-update-clause), [delete](/docs/sqlite/delete#with-delete-clause)
</Callout>
...
values (?, ((select * from "user_count") = 0)) 
returning "admin"
```
</Section>

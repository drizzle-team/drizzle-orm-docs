...
<Callout type="info">
Read more on how to upgrade [here](/docs/sqlite/upgrade-v1)
</Callout>

## Breaking changes

#### Foreign key handling in pre-v1.0
...
After upgrading to v1, the first `push` will produce a diff that recreates the table (DB has no name, TS does). **To avoid this, remove the FK name from the TS schema** - no diff will be generated.

`drizzle-kit generate` behaves as expected; no changes needed.

#### Relational Queries v1 removed

RQBv1 has been removed. Use [Relational Queries v2](/docs/sqlite/relations-v2) with the new `defineRelations()` API instead and new features. You can find more details about migrating from rqb v1 to v2 [here](/docs/sqlite/relations-v1-v2)
...
The `--strict` flag has been removed — its behavior is now the default. `drizzle-kit push` always prompts for confirmation for data-loss statements unless the `--force` flag is passed. Use [drizzle-kit push --explain](/docs/sqlite/drizzle-kit-push) to preview the SQL that would be executed before running it.
...
You can find out more [here](/docs/sqlite/jit-mappers)
...
New MSSQL dialect support added. You can read more [here](/docs/sqlite/mssql/get-started-mssql)

#### New CockroachDB dialect

New CockroachDB dialect support added. You can read more [here](/docs/sqlite/cockroach/get-started-cockroach)

#### Column aliases via `.as()`

You can find out more aliasing info [here](/docs/sqlite/aliases)
```ts
const query = db
...
Creates the drizzle migration table and marks the first pulled migration as applied. You can read more [here](/docs/sqlite/drizzle-kit-pull)

#### `drizzle-kit push --explain`

Shows the SQL that would be executed without actually running it. You can read more [here](/docs/sqlite/drizzle-kit-push)
...
`drizzle-kit generate` now identifies [incompatible changes](/docs/sqlite/v1-upgrade#drizzle-kit-check--commutativity-check) across migration branches. Use `--ignore-conflicts` to bypass.
...
| `id` | integer | yes | yes |
| `hash` | text | yes | yes |
| `created_at` | numeric | yes | yes (legacy) |
| `name` | text | — | new |
| `applied_at` | text | — | new |

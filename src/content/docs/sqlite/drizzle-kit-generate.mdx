...
It's designed to cover [code first](/docs/sqlite/migrations) approach of managing Drizzle migrations. 
You can apply generated migrations using [`drizzle-kit migrate`](/docs/sqlite/drizzle-kit-migrate), using drizzle-orm's `migrate()`, 
...
you can set them either via [drizzle.config.ts](/docs/sqlite/drizzle-config-file) config file or via CLI options
...
for DDL alternations currently not supported by Drizzle Kit or data seeding. Extended docs on custom migrations - [see here](/docs/sqlite/kit-custom-migrations)

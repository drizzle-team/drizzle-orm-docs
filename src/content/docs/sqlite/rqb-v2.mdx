...
Relational queries are an extension to Drizzle's original **[query builder](/docs/sqlite/select)**.
...
<Callout type="info" emoji="ℹ️">
	`drizzle` import path depends on the **[database driver](/docs/sqlite/connect-overview)** you're using.
</Callout>
...
For any nested `with` queries Drizzle will infer types using [Core Type API](/docs/sqlite/goodies#type-api).
...
relational queries API lets you define filters and conditions with the list of our **[`operators`](/docs/sqlite/operators)**.  
...
export const users = sqliteTable("users", {
  ...
  preferences: text("preferences", { mode: "json" }), // JSON column for user settings/preferences
  interests: text("interests"),                       // Comma-separated user interests
});
```
```ts
const response = db.query.users.findMany({
...
You can use same ordering **[core API](/docs/sqlite/select#order-by)** or use
...
<Callout type="warning" emoji="⚠️">
	As of now aggregations are not supported in `extras`, please use **[`core queries`](/docs/sqlite/select)** for that.
</Callout>
...
```sql
select "d0"."id" as "id", "d0"."name" as "name",
((select coalesce(json_group_array(json_object('id', "d1"."id", 'content', "d1"."content", 'authorId', "d1"."author_id")), json_array())
...
### Prepared statements
Prepared statements are designed to massively improve query performance — [see here.](/docs/sqlite/perf-queries)

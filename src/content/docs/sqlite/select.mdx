...
and whatever it doesn't support yet, can be added by the user with the powerful [`sql`](/docs/sqlite/sql) operator.
...
If you need to apply runtime transformations to the returned value, you can use the [`.mapWith()`](/docs/sqlite/sql#sqlmapwith) method.
</Callout>
...
Sneak peek of advanced partial select, for more detailed advanced usage examples - see our [dedicated guide](/docs/sqlite/guides/include-or-exclude-columns).
...
You can filter the query results using the [filter operators](/docs/sqlite/operators) in the `.where()` method:
...
All filter operators are implemented using the [`sql`](/docs/sqlite/sql) function.
...
Sneak peek of conditional filtering, for more detailed advanced usage examples - see our [dedicated guide](/docs/sqlite/guides/conditional-filters-in-query).
...
usage examples - see our dedicated [limit offset pagination](/docs/sqlite/guides/limit-offset-pagination)
and [cursor pagination](/docs/sqlite/guides/cursor-based-pagination) guides.
...
<Callout>
  Check how to use WITH statement with [insert](/docs/sqlite/insert#with-insert-clause), [update](/docs/sqlite/update#with-update-clause), [delete](/docs/sqlite/delete#with-delete-clause)
</Callout>
...
```
```sql
with "sq" as (delete from "users" where "users"."name" = ? returning "id", "name", "age") 
select "id", "name", "age" from "sq"
```
</Section>
...
<Callout type="info">
`cast(... as int)` keeps the selected count value typed as a number.
Alternatively, you can use [`.mapWith(Number)`](/docs/sqlite/sql#sqlmapwith) to cast the value to a number at runtime.

If you need count aggregation - we recommend using our [`$count`](/docs/sqlite/select#count) API
</Callout>

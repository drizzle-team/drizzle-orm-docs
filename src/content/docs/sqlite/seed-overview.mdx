...
Different dialects will have different strategies for database resetting. For SQLite:
...
There are many more possibilities that we will define in these docs, but for now, you can explore a few sections in this documentation. Check the [Generators](/docs/sqlite/seed-functions) section to get familiar with all the available generator functions you can use.
...
```ts filename="schema.ts"
import { sqliteTable, integer, text, text, real } from "drizzle-orm/sqlite-core";
...
import { integer, real, sqliteTable, text, integer, text } from "drizzle-orm/sqlite-core";

export const customers = sqliteTable('customer', {
	id: text({ length: 256 }).primaryKey(),
...
export const employees = sqliteTable(
	'employee',
	{
		...
		notes: text().notNull(),
		reportsTo: integer().references((): AnyColumn => employees.id),
		photoPath: text(),
	},
);
...
export const orders = sqliteTable('order', {
	id: integer().primaryKey({ autoIncrement: true }),
	orderDate: integer().notNull(),
	requiredDate: integer().notNull(),
	shippedDate: integer(),
	shipVia: integer().notNull(),
	freight: real().notNull(),

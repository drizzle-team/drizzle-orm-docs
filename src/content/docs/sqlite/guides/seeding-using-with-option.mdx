...
- Get familiar with [One-to-many Relation](/docs/sqlite/relations#one-to-many)
- Get familiar with [Drizzle Seed](/docs/sqlite/seed-overview)
</Prerequisites>
...
export const postsRelations = defineRelations({ posts, users }, (r) => ({
  posts: {
	author: r.one.users({
...
export const users = sqliteTable('users', {
	id: integer().primaryKey({ autoIncrement: true }),
	name: text(),
    reportsTo: integer('reports_to').references((): AnySQLiteColumn => users.id),
});
```
</CodeTab>
</CodeTabs>

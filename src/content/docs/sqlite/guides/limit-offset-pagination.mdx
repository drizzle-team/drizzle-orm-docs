...
- [Select statement](/docs/sqlite/select) with [order by clause](/docs/sqlite/select#order-by) and [limit & offset clauses](/docs/sqlite/select#limit--offset)
- [Relational queries](/docs/sqlite/rqb) with [order by clause](/docs/sqlite/rqb#order-by) and [limit & offset clauses](/docs/sqlite/rqb#limit--offset)
- [Dynamic query building](/docs/sqlite/dynamic-query-building)
</Prerequisites>
...
import { SQLiteColumn, type SQLiteSelect } from 'drizzle-orm/sqlite-core';

function withPagination<T extends SQLiteSelect>(
  qb: T,
  orderByColumn: SQLiteColumn | SQL | SQL.Aliased,
  page = 1,
  pageSize = 3,
) {
  return qb
...
So, if your database experiences frequently insert and delete operations in real time or you need high performance to paginate large tables, you should consider using [cursor-based](/docs/sqlite/guides/cursor-based-pagination) pagination instead.

...
`postgresql`,`mysql` or `sqlite` | | `out` | | migrations folder,
default=`./drizzle` | | `config` | | config file path,
default=`drizzle.config.ts` |
<br />
<Npx>
  drizzle-kit check --dialect=postgresql drizzle-kit check --dialect=postgresql

...
1. Each professor is assigned to teach only one specific course subject.
...
`professor` is a non-prime attribute because it's not part of any candidate key.

### 3NF Analysis

The non-prime attribute `professor` depends on every candidate key (every = only in this case). `course_name` is a prime attribute because it is part of the only candidate key `student_id, course_name`. There is no transitive dependency.
...
2. Update anomalies. If the name of a course taught by a professor changes, we would need to update multiple rows to reflect the change.
3. Deletion anomalies. If students drop a course, we would lose information about the course and the professor.
...
We have achieved BCNF by splitting the table into two tables: `professors` and `students_professors`. Now, the tables are free from functional dependencies that violate BCNF.

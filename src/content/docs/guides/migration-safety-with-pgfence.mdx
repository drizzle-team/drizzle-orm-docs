...
```bash copy
npx @flvmnt/pgfence analyze drizzle/*.sql
```
...
Running `npx @flvmnt/pgfence analyze drizzle/0001_add_age.sql` produces output like:

<Section>
```bash copy
npx @flvmnt/pgfence analyze drizzle/0001_add_age.sql
```
  ...
  pgfence:
    ...
    steps:
      ...
      - name: Analyze migrations
        run: npx @flvmnt/pgfence analyze --ci --max-risk medium --output github drizzle/*.sql
```
...
npx @flvmnt/pgfence analyze --output json drizzle/*.sql

# GitHub PR comment markdown
npx @flvmnt/pgfence analyze --output github drizzle/*.sql
```

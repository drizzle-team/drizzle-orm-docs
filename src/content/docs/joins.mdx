...
```
```sql
select ... from "users" left join lateral (select ... from "pets" where "users"."age" >= 16) as "userPets" on true
```
```typescript
// result type
const result: {
...
```
```sql
select ... from "users" inner join lateral (select ... from "pets" where "users"."age" >= 16) as "userPets" on true
```
```typescript
// result type
const result: {
...
```typescript copy
const subquery = db.select().from(pets).where(gte(users.age, 16)).as('userPets')
const result = await db.select().from(users).crossJoinLateral(subquery, sql`true`)
```
```sql
select ... from "users" cross join lateral (select ... from "pets" where "users"."age" >= 16) as "userPets"
```
```typescript
// result type
const result: {

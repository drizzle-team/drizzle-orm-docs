...
export class ShikiLine {
  ...
  constructor(highlightedCodeLine: string) {
    ...
    const tokensMatchesLength = [...tokensHtml.matchAll(tokenRegExp)].length;
    this.tokens = [];
    this.textLine = "";
    if (tokensMatchesLength === 0) {
      const spanRegExp = /<span[^>]*>(.*?)<\/span>/g;
      const tokenMatches = [...tokensHtml.matchAll(spanRegExp)];
    ...
    this.tokens.forEach((token) => {
      ...
      if (inInlineMarker || isLineMarked) {
        if (!token.color.startsWith("var(")) {
          ...
          const fixedTokenColor = ensureTextContrast(
            tokenColor,
            backgroundColor,
          );
  ...
  private getInlineMarkingDefinitionMatches(
    ...
    if (inlineMarking.regExp) {
      ...
      for (const match of matches) {
        ...
        let groupIndices = (match as any).x as (
